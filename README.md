...
- Start 버튼 뒤 스크림은 Figma 값 그대로 **layer blur 62 + background blur 2**다. 62px는
  스크림 자체에 걸린 블러라서 경계선 없이 프레임에 녹아든다. 스크림 색은 `--bg`를 따라가므로
  배경색을 흰 책상으로 바꿔도 회색 띠가 남지 않는다
...
- 숫자는 스냅하지 않고 **굴러 올라간다**. 스텝 진입 시 Total이 0→값까지 700ms,
