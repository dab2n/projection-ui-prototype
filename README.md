...
- Start 버튼 뒤 스크림은 Figma 값 그대로 **layer blur 62 + background blur 2**이고, 그 위에
  `blur.svg` 글로우가 얹힌다. 62px는 스크림 자체에 걸린 블러라서 경계선 없이 프레임에 녹아든다.
  스크롤 영역을 프레임 바닥까지 늘린 뒤로는 62px 블러만으로는 아래 프레임들이 계속 비쳐서,
  버튼 아래부터는 `--bg` 단색을 한 겹 더 덮는다 (위쪽은 마스크로 페이드인). 스크림 색은 모두
  `--bg`를 따라가므로 배경색을 흰 책상으로 바꿔도 회색 띠가 남지 않는다
...
- 불꽃은 프로필 근처에서 **점점 멀어지며 위로 흘러가고** 사라진다 (3.4s, 두 개가 1.5s 엇갈림).
