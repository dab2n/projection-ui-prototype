...
노브 세 개는 스크린샷만으로는 알 수 없는 값들이라 열어뒀다:

| 노브 | 뜻 | 기본 |
|------|-----|------|
| 책상 밝기 | 카메라에 찍히는 책상의 값. 255가 아니다 — 그 위 여유분이 투사가 쓸 수 있는 대비 전부다 | 216 |
| 채도 | 확산 백색면이 먹는 채도 | 65% |
| 블랙 누광 | 프로젝터의 블랙 레벨 유출. 실제 장비는 0이 되지 않는다 | 10 |

켜 보면 바로 드러나는 것: **흰 카드와 흰 텍스트가 함께 사라지고, 검정 글자는 아예 없다.**
지금 디자인의 명암 극성이 흰 책상과 반대라는 뜻이다. 살아남는 건 채도 있는 레드/오렌지뿐이므로,
대비는 명도가 아니라 **색상·채도**로 다시 짜야 한다. 실제 합성 때도 Normal이 아니라
Screen(Add) 블렌드를 쓰고, 프로젝터가 순검정을 출력한 상태의 책상 플레이트를 먼저 찍어
그것을 베이스로 쓰면 대비가 실제와 맞는다.
